...
| "next day top" / "明天买啥" / "次日预测" | `.\Get-NextDayTop1.ps1` |
...
- **腾讯股票 (Tencent)** web.ifzq.gtimg.cn：**K线主源**（日K前复权），qt.gtimg.cn 实时行情备源
- **东方财富 (East Money)** push2 API：行业板块、概念板块、板块成分股实时行情、资金流；push2his K线（备源）；emweb/datacenter 财报
- **新浪财经 (Sina Finance)** ：滚动新闻 API + 美股/期货实时行情（hq.sinajs.cn）+ A股K线
- **雪球 / 36Kr**：情绪分析
- **Akshare (Python)**：A 股历史财务数据（`C:\Python\Python39\python.exe`，需 `PYTHONIOENCODING=utf-8`）

### API 降级策略

东财 API 盘中经常 `ResponseEnded` / 超时，AlphaSignal 内置自动降级：
- **K线**：腾讯（主源 10s）→ 东财 push2his（备源 8s，连续失败2次自动跳过）
- **财报**：东财 emweb（主源 10s）→ 东财 datacenter（备源 12s，连续失败2次自动跳过）
- **板块/成分股**：仍用东财 push2（暂无备源），失败时成分股池变小但流程不中断

## 脚本总览

| 脚本 | 功能 | 典型耗时 |
|------|------|----------|
| `Get-AlphaSignal.ps1` | **主入口**：6 段式全市场分析 + AI TOP10 选股 | ~3.5 分钟（含腾讯备源） |
...
| `Get-NextDayTop1.ps1` | 次日 TOP1 低开高走预测（每小时更新 / 单次扫描） | 1~3 分钟 |
...
| "完整选股" | `Get-AlphaSignal`（完整编排器） | ~3.5min |
...
| 维度 | 分值范围 | 主要指标 |
|------|----------|----------|
| 基本面 | 0~40 | 营收增速 + 净利增速 + ROE + 毛利率 + **PEG** + **财报趋势** |
| 技术面 | **-10~30** | 回调幅度 + RSI + **RSI过热惩罚** + **追高惩罚** + **破位惩罚** |
| 估值面 | **-10~30** | CAPE/PE/PB + 相对行业PE + **极端PE惩罚** + **极端PB惩罚** |

**PEG 加分**：PE(TTM) ÷ 净利润增速。PEG<0.5 +8分、0.5~1.0 +5分、1.0~1.5 +2分、>3.0 -3分。
**相对行业PE**：PE/行业中位PE < 0.6 +7分 / < 0.8 +5分 / < 0.9 +2分 / > 1.3 -3分。
...
| →整 | >MA20 但 <MA60 | 整理反弹 |
| ↓弱 | <MA20 但 >MA60 | 短期走弱 |
| ↓破 | <MA20 且 <MA60 | 双破位（已被硬过滤） |

### 信号类型

| 信号类型 | 判断条件 | 加分 | 建议持有时长 | 仓位参考 |
|---------|---------|------|------------|----------|
| 价值洼地 | CAPE 低估 / PE 低于行业均值 80% / PE<15 且盈利 / PB<1.5 | +8 | 长线 3-6月 | 15-20% |
| 景气反转 | 营收增速 > 25% 且 净利增速 > 25% | +5 | 波段 1-2月 | 10-15% |
| 主题热点 | 其余情况 | **-3** | 短线 1-2周 | 5-10% |

止损参考 = 当前价 × 92%（下跌 8%）。

### 短线动量信号（独立通道）

当 Alpha 候选为空或不足时，额外输出动量信号：
- 条件：日涨>3% + 放量(>5日均量×1.2) + 站上MA20 + RSI<80
- 评分：日涨(0-40) + 放量(0-20) + 板块热度(0-25) + 技术(0-15)，满分100
- 止损：当前价 × 95%（-5%），仓位 5-10%，持有 1-2 周
